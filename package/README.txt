...
For volume latency panels compatible with the legacy `VolumePerfHandler`, use
the `apiontap:perf` events generated by the Performance metric:

```spl
index=ontap sourcetype=apiontap:perf source=VolumePerfHandler
...
| eval avg_latency_average=round(avg_latency_average/1000,2),
       avg_latency_max=round(avg_latency_max/1000,2)
| eval vserver_name=if(vserver_name="None", "", vserver_name)
| where avg_latency_average > 20
| sort - avg_latency_max
| fields host, vserver_name, instance_name, avg_latency_average, avg_latency_max
| rename host AS "Host", instance_name AS "Volume", vserver_name AS "VServer",
         avg_latency_average AS "Average Latency", avg_latency_max AS "Max Latency"
| head 100
```

The REST-native names (`name`, `uuid`, `svm_name`, and
`average_latency_average`) remain available on the same events.
...
| `apiontap:perf` | REST performance counter rows from `/api/cluster/counter/tables` |
